...
export function Topbar({ onMobileMenuClick }: TopbarProps) {
  ...
  return (
    <>
      <header className="sticky top-0 z-20 h-16 bg-white border-b border-slate-200/80 px-3 sm:px-8 flex items-center justify-between gap-3 select-none">
        {/* Left: Sidebar Toggle & Modern Search Bar */}
        <div className="flex items-center gap-2 sm:gap-4 flex-1 max-w-xl">
          ...
          >
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <span className="truncate text-[11px] sm:text-xs">Search or type command...</span>
            </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl border border-blue-200 bg-blue-50/80 hover:bg-blue-100 text-blue-700 text-xs font-semibold transition-all shadow-2xs"
            title="View Latest System Updates & Bug Fixes"
          >
            <Sparkles className="w-3.5 h-3.5 text-blue-600" />
            <span className="hidden md:inline">What&apos;s New</span>
            <span className="px-1 py-0.2 rounded text-[9px] font-black bg-blue-600 text-white">v2.1</span>
          </button>
      ...
      {/* Manual What's New Updates Modal */}
      <OnboardingModal
        forceOpen={updatesModalOpen}
